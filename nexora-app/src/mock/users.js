...
export const SEED_USERS = [
  { _id: 'user_001', name: 'Tanishq Patel', email: 'demo@nexora.com', profilePicture: null, workspaceRole: 'owner',  projectRole: 'project_manager', isActive: true,  lastSeen: new Date().toISOString(),               createdAt: '2024-01-15T09:00:00.000Z' },
...
export const SEED_CREDENTIALS = [
  ...
  { email: 'demo@nexora',        password: 'password123' },
...
export function getUsers() {
  const users = loadFromStorage(STORAGE_KEY_USERS, null);
  if (!users || users.some(u => u._id === 'user_000' || u.name === 'Demo User' || u.name.includes('Sharma') || u.name.includes('Mehta') || u.email === 'tanishq@nexora.com')) {
...
export function getCredentials() {
  const creds = loadFromStorage(STORAGE_KEY_CREDENTIALS, null);
  if (!creds || !creds.some(c => c.email === 'demo@nexora' || c.email === 'demo@nexora.com')) {
    saveCredentials(SEED_CREDENTIALS);
...
export function saveCredentials(credentials) {
  saveToStorage(STORAGE_KEY_CREDENTIALS, credentials);
}
